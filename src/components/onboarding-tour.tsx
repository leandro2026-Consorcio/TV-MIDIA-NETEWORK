...
const steps = [
  ...
  { title: 'Faça o pareamento', text: 'A TV exibirá um código. Digite esse código no painel para vincular a TV à sua empresa.', icon: Radio, href: '/screens', action: 'Parear TV' },
...
export function OnboardingTour({ autoOpen = false }: TourProps) {
  const [open, setOpen] = useState(false);
  const [step, setStep] = useState(0);
  const [dontShow, setDontShow] = useState(false);

  const show = useCallback((automatic = false) => {
    setStep(0);
    setOpen(true);
  ...
  useEffect(() => {
    if (autoOpen) show(true);
  }, [autoOpen, show]);

  useEffect(() => {
    const listener = () => show(false);
  ...
  function next() {
    ...
    if (step === steps.length - 1) void close('complete');
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/80 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="tour-title">
      <div className="w-full max-w-xl overflow-hidden rounded-3xl border border-sky-500/20 bg-slate-900 shadow-2xl">
        <div className="h-1 bg-slate-800"><div className="h-full bg-sky-500 transition-all" style={{ width: `${((step + 1) / steps.length) * 100}%` }} /></div>
        <div className="p-6 sm:p-8">
          ...
          <p className="mt-6 text-[11px] font-bold uppercase tracking-wider text-sky-400">Etapa {step + 1} de {steps.length}</p>
          ...
          {current.href && <Link href={current.href} className="mt-5 inline-flex rounded-xl border border-purple-500/20 bg-purple-500/10 px-4 py-2.5 text-xs font-bold text-purple-300 hover:bg-purple-500/20">{current.action}</Link>}
