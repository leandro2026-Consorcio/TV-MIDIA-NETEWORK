...
export function InformativeContentCard({ item }: { item: PlayerPlaylistItem }) {
  ...
  return (
    <article className="relative flex h-full w-full overflow-hidden bg-gradient-to-br from-slate-950 via-slate-900 to-sky-950 text-white">
      ...
      <div className={`relative z-10 flex h-full w-full ${showImage ? 'grid grid-cols-[1.25fr_0.75fr]' : ''}`}>
        <div className="flex min-w-0 flex-col justify-center px-[7vw] py-[8vh]">
          ...
          <h1 className="max-w-5xl text-balance text-[clamp(2.4rem,5vw,5.8rem)] font-black leading-[1.04] tracking-tight">
            {item.title}
          </h1>
          {item.summary && (
            <p className="mt-7 max-w-5xl text-pretty text-[clamp(1.2rem,2.2vw,2.25rem)] leading-relaxed text-slate-200">
              {item.summary}
            </p>
          )}

          <footer className="mt-10 flex flex-wrap items-center gap-x-4 gap-y-2 text-[clamp(0.75rem,1.1vw,1rem)] font-medium text-slate-400">
