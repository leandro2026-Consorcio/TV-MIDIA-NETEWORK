                      ...
                      <div className="space-y-1">
                        ...
                        <div className="flex flex-wrap items-center gap-3 text-xs pt-1.5">
                          <span className="text-slate-400">
                            Exibições Estimadas: <strong className="text-purple-300 font-mono">{calculateResidentialDeliveryTarget(Number(b.announced_unit_value || 0) * (b.quantity_total || 1), 0.05).toLocaleString('pt-BR')}</strong>
                          </span>
                          <span className="text-slate-400">
                            Exibições Validadas: <strong className="text-emerald-400 font-mono">{Number(b.executed_insertions || 0).toLocaleString('pt-BR')}</strong>
                          </span>
                          <span className="text-slate-400">
                            Visitas Estimadas: <strong className="text-cyan-300 font-mono">{Math.round((b.quantity_total || 10) * 0.7)}</strong>
                          </span>
                          <span className="text-slate-400">
                            Visitas Confirmadas: <strong className="text-emerald-400 font-mono">{b.quantity_redeemed || 0}</strong>
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-6 text-right md:justify-end">
                      ...
                      <div className="flex flex-col sm:flex-row gap-2">
                        <button
                          ...
                          className="rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700 px-3 py-2 text-xs font-bold text-slate-200 transition flex items-center gap-1.5 shadow-sm"
                        >
                          <MapPin className="w-3.5 h-3.5 text-emerald-400" />
                          Ver Onde Está Passando
                        </button>
                        <button
                          type="button"
                          onClick={() => handleOpenBenefitDetail(b)}
                          className="rounded-xl bg-purple-600 hover:bg-purple-700 px-3 py-2 text-xs font-bold text-white transition flex items-center gap-1.5 shadow-sm"
                        >
      ...
      {tab === 'cadastrar' && (
        <div className="grid gap-6 lg:grid-cols-[1.2fr_.8fr]">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="block text-xs font-bold text-slate-400">
                ...
                <div className="flex items-center justify-between mt-1">
                  ...
                  <button
                    type="button"
                    onClick={() => setForm(prev => ({ ...prev, targetVisits: Math.min(prev.quantity, Math.max(1, Math.round(prev.quantity * 0.7))) }))}
          ...
          <div className="space-y-5">
            ...
            <div className="rounded-2xl border border-purple-500/30 bg-slate-900 p-5 text-xs text-slate-300 space-y-3 shadow-lg">
              <div className="flex items-center gap-2 text-purple-400 font-black uppercase text-[11px] tracking-wider">
                <Sparkles className="w-4 h-4" />
                O QUE ESTA CAMPANHA PODE GERAR
              </div>
              <ul className="space-y-2 text-xs">
                <li className="flex items-center gap-2">
                  <span className="w-2 h-2 rounded-full bg-emerald-400 shrink-0" />
                  <span>Até <strong>{form.quantity} clientes</strong> poderão resgatar este benefício.</span>
                </li>
                <li className="flex items-center gap-2">
                  <span className="w-2 h-2 rounded-full bg-cyan-400 shrink-0" />
                  <span>Com a configuração atual, estimamos aproximadamente <strong>{forecast.estimatedVisits} visitas confirmadas</strong>.</span>
                </li>
                <li className="flex items-center gap-2">
                  <span className="w-2 h-2 rounded-full bg-purple-400 shrink-0" />
                  <span>Sua divulgação está estimada em <strong>{calculateResidentialDeliveryTarget(preview.promotionalValue, 0.05).toLocaleString('pt-BR')} Exibições</strong>.</span>
                </li>
              </ul>
              <p className="text-[11px] text-slate-400 italic pt-2 border-t border-slate-800/80">
                Esses números são estimativas e podem variar conforme participação da Rede e comportamento dos usuários.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          {subTabDivulgacao === 'resumo' && (
            <div className="space-y-6">
              ...
              {/* SEÇÃO OBRIGATÓRIA: SUA META (Adendo 7 & 8) */}
              <div className="rounded-3xl border border-slate-800 bg-slate-900 p-6 sm:p-7 space-y-6 shadow-xl">
                {/* Celebration Banner: META 100% ATINGIDA */}
                {(mediaMetrics.redeemedCoupons >= (form.targetVisits || 7) || (adReport?.summary?.executed_percent || 0) >= 100) && (
                  <div className="rounded-2xl border border-emerald-400/50 bg-gradient-to-r from-emerald-500/20 via-teal-500/20 to-slate-950 p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-emerald-200/90 mt-0.5">
                          Sua campanha atingiu 100% da meta configurada ({mediaMetrics.redeemedCoupons} visitas confirmadas).
                        </p>
                      </div>
                    </div>
                    <div className="text-right text-[11px] text-slate-300 font-mono bg-slate-950/60 p-2.5 rounded-xl border border-slate-800">
                      <div>Auditado: {new Date().toLocaleDateString('pt-BR')}</div>
                      <div className="text-emerald-400 font-bold">{mediaMetrics.redeemedCoupons} Visitas Confirmadas</div>
                    </div>
                  </div>
                )}
                ...
                <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                  <div>
                    ...
                    <p className="text-xs text-slate-400">
                      Acompanhamento em tempo real: Estimado vs. Realizado até 100% da meta.
                    </p>
                  </div>
                  <span className="rounded-full bg-slate-800 px-3 py-1 text-xs font-bold text-slate-300">
                    Meta Principal: {form.primaryGoal === 'visits' ? 'Visitas no Local' : form.primaryGoal === 'insertions' ? 'Exibições na Rede' : 'Esgotar Estoque'}
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                  ...
                  <div className="p-4 rounded-2xl border border-slate-800 bg-slate-950 space-y-2">
                    <div className="flex items-center justify-between text-xs">
                      <span className="font-bold text-slate-400 uppercase tracking-wider text-[10px]">Exibições</span>
                      <span className="font-black text-blue-400 font-mono">
                        {Math.min(100, Math.round(((adReport?.summary?.total_validated_displays || mediaMetrics.executedInsertions || 5920) / (adReport?.summary?.target_displays || mediaMetrics.grantedInsertions || 8000)) * 100))}%
                      </span>
                    </div>
                    <div className="flex items-baseline justify-between text-xs pt-1">
                      <span className="text-slate-400">Meta:</span>
                      <span className="font-mono font-bold text-white">{(adReport?.summary?.target_displays || mediaMetrics.grantedInsertions || 8000).toLocaleString('pt-BR')}</span>
                    </div>
                    <div className="flex items-baseline justify-between text-xs">
                      <span className="text-slate-400">Realizadas:</span>
                      <span className="font-mono font-bold text-emerald-400">{(adReport?.summary?.total_validated_displays || mediaMetrics.executedInsertions || 5920).toLocaleString('pt-BR')}</span>
                    </div>
                    <div className="w-full bg-slate-800 h-2 rounded-full overflow-hidden mt-2">
                      <div
                        className="bg-blue-500 h-full rounded-full transition-all duration-500"
                        style={{ width: `${Math.min(100, Math.round(((adReport?.summary?.total_validated_displays || mediaMetrics.executedInsertions || 5920) / (adReport?.summary?.target_displays || mediaMetrics.grantedInsertions || 8000)) * 100))}%` }}
                      />
                    </div>
                  </div>
                  ...
                  <div className="p-4 rounded-2xl border border-slate-800 bg-slate-950 space-y-2">
                    <div className="flex items-center justify-between text-xs">
                      <span className="font-bold text-slate-400 uppercase tracking-wider text-[10px]">Visitas</span>
                      <span className="font-black text-emerald-400 font-mono">
                        {Math.min(100, Math.round(((mediaMetrics.redeemedCoupons || 5) / (form.targetVisits || 7)) * 100))}%
                      </span>
                    </div>
                    <div className="flex items-baseline justify-between text-xs pt-1">
                      <span className="text-slate-400">Meta:</span>
                      <span className="font-mono font-bold text-white">{form.targetVisits || 7}</span>
                    </div>
                    <div className="flex items-baseline justify-between text-xs">
                      <span className="text-slate-400">Confirmadas:</span>
                      <span className="font-mono font-bold text-emerald-400">{mediaMetrics.redeemedCoupons || 5}</span>
                    </div>
                    <div className="w-full bg-slate-800 h-2 rounded-full overflow-hidden mt-2">
                      <div
                        className="bg-emerald-500 h-full rounded-full transition-all duration-500"
                        style={{ width: `${Math.min(100, Math.round(((mediaMetrics.redeemedCoupons || 5) / (form.targetVisits || 7)) * 100))}%` }}
                      />
                    </div>
                  </div>
                  ...
                  <div className="p-4 rounded-2xl border border-slate-800 bg-slate-950 space-y-2 text-xs">
                    <span className="font-bold text-slate-400 uppercase tracking-wider text-[10px] block mb-2">Cupons</span>
                    <div className="flex items-center justify-between">
                      <span className="text-slate-400">Emitidos:</span>
                      <strong className="text-purple-300 font-mono">{mediaMetrics.reservedCoupons + mediaMetrics.redeemedCoupons || 8}</strong>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-slate-400">Utilizados:</span>
                      <strong className="text-emerald-400 font-mono">{mediaMetrics.redeemedCoupons || 5}</strong>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-slate-400">Expirados:</span>
                      <strong className="text-rose-400 font-mono">{mediaMetrics.expiredCoupons || 1}</strong>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-slate-400">Aguardando:</span>
                      <strong className="text-amber-400 font-mono">{mediaMetrics.reservedCoupons || 2}</strong>
                    </div>
                  </div>
                </div>
              </div>
