...
INSERT INTO public.help_articles (slug, audience, title, objective, expected_result, steps, display_order) VALUES
  ...
  ('onde-encontro-meu-cupom', 'organic', 'Onde encontro meu cupom', 'Localizar os cupons já emitidos para apresentar na loja.', 'Visualização imediata do cupom com código legível e QR Code.', ARRAY['Acesse a seção Meus Cupons na página de Prêmios Orgânicos', 'Localize o cupom com status Aguardando Utilização', 'Apresente a tela do celular com o QR Code para o atendente'], 61),
  ('quando-meus-pontos-voltam', 'organic', 'Quando meus pontos voltam', 'Saber como funciona o estorno em caso de expiração sem uso.', 'Microcréditos devolvidos integralmente para o saldo disponível via ledger.', ARRAY['Se você não utilizar o cupom antes da data limite, ele expirará', 'A rotina automática realiza o estorno dos pontos para seu saldo', 'Você poderá usá-los novamente para resgatar outro benefício'], 62),
  ('meu-cupom-expirou', 'organic', 'Meu cupom expirou', 'Entender o que ocorre quando a validade de 7 dias termina.', 'Cupom marcado como expirado e pontos estornados de acordo com a política.', ARRAY['Cada cupom possui um prazo determinado após a emissão', 'Após a data e horário limite, o cupom não pode mais ser aceito no estabelecimento', 'Verifique seu saldo para confirmar o estorno dos pontos'], 63),
  ('onde-posso-usar-o-beneficio', 'organic', 'Onde posso usar o benefício', 'Identificar os endereços e unidades participantes.', 'Lista clara de unidades e cidades no detalhe do cupom.', ARRAY['Abra os detalhes do seu cupom', 'Consulte a seção Unidades Participantes', 'Observe as regras de dias da semana (ex.: terça a quinta) e horários'], 64)
