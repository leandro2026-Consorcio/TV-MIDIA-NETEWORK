-- Campaign DATE fields are civil dates in the platform's operational timezone.
-- CURRENT_DATE follows the database UTC session and can expire a campaign three
-- hours too early for America/Cuiaba.
...
BEGIN
  ...
  IF v_campaign.end_date IS NOT NULL AND v_campaign.end_date < (now() AT TIME ZONE 'America/Cuiaba')::date THEN RETURN jsonb_build_object('success',false,'error','Esta campanha já terminou.'); END IF;
